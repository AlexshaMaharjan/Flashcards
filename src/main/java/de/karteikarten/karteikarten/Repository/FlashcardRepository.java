...
public class FlashcardRepository {
    ...
    public int createCard(Flashcard card) {
    ...
    public List<Flashcard> findFlashcardsByTagIds(List<Integer> tagIds) {
        List<Flashcard> flashcards = new ArrayList<>();
        if (tagIds.isEmpty()) {
            return flashcards;
        }
        ...
        String sql = "SELECT f.* FROM FLASHCARDS f INNER JOIN FLASHCARD_TAGS ft ON f.ID = ft.FLASHCARD_ID WHERE ft.TAG_ID IN (" + inClause + ")";

        System.out.println("Datenbankfehler beim Abrufen gefilterter Karteikarten: " + tagIds);
        ...
        try (Connection conn = Database.connect();
            ...
            System.out.println("Datenbankfehler beim Abrufen gefilterter Karteikarten: " + e.getMessage());
        }
        return flashcards;
    }
